...
function git_commit_fixup(){ git commit -a --fixup HEAD; git_log_short origin/master..HEAD }
...
function git_log_short() { git log --format='%h %f' ${1:-HEAD} | tee >(wc -l) |cat  | head -30 } # name of the author is %an
function git_log_master(){ git_log_short origin/master..${1:-HEAD} }
alias glm='git_log_master'
