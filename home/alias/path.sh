alias lp=list_path # return a list of saved path indexed. # ex. `lp` -> return all path from .ignore/path1.txt...ignore/pathN.txt
function list_path(){ for path_file in .ignore/path*.txt; do echo "## ${path_file}";cat ${path_file} | pyp 'pp'; done}
alias lt=list_test # ex. `$ lt`  ->  return list of path  in .ignore/path*.txt that match the regex test
